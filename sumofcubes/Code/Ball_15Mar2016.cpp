				   ...
				   BOOL bTestBall, BOOL bIsBot)
{
	...
	if(!bTestBall)
	{
		...
		sprintf(buf, "cuetip: %f, rnd: %f", cuetip, rnd);
		ErrorMessage(m_parent->m_parent, buf);
...
*/
	...
	*/
	...
	{
		char buffer[64];
		sprintf(buffer, "%d", m_WDelay);
		ErrorMessage(m_parent->m_parent, buffer);
	}
	*/
...
{		
	...
	if (g_gameAppView->m_replay->m_version >= 1003) //iPool
	{
		factor = cmag;
		factor = factor / maxspeed;
		factor = 0.99 - factor * 0.30f;
	}
