...
#define PLAYERCOPY_STATS	(PLAYERCOPY_RATINGS | PLAYERCOPY_BREAKS | PLAYERCOPY_GAMES | PLAYERCOPY_LEVEL)
///////
...
public:
	...
	void Clear(void);
	void Add(char *stid, char *sRaceTo, char *name1P, char *name2P, char *sfp1, char *sfp2, char *swpts, char *slpts, char *sfee1, char *sfee2, char *sscore1, char *sscore2, char *sround, char *sWACCEXT, char *sLACCEXT, char *sPen, int idx);
	void NotSeatedInvite(void);
};

#define MAX_TOURNEYS			16
#define MAX_TMATCHES			100		// should be up to 32 matches on 32 tables, if we're only doing last 64 players
#define MATCH_TIMETOSITSTART	5*60	// 10mins
...
public:
	...
	void AddMatch(char *stid, char *sRaceTo, char *name1P, char *name2P, char *sfp1, char *sfp2, char *swpts, char *slpts, char *sfee1, char *sfee2, char *sscore1, char *sscore2, char *sround, char *sWACCEXT, char *sLACCEXT, char *sPen, char *sGameType, int idx);
...
public:
	...
	void ServerDataReceive(BOOL bTournament, char handle[22]);
	...
	BOOL PressedConcedeReceive(int plrSockIdx, BOOL bAndExit = FALSE);
	...
	void ServerGivePlayerPts(char *name, int pts, int round = -1);
	...
	void UpdateTableHostDetails(int cueError, BOOL bRated, char type, BOOL bMissRule, int playerSockIdx,
								...
								int tournamentTable = -1, int fp1 = 0, int fp2 = 0, int score1 = 0, int score2 = 0);
	...
	void UpdateTableItemButtons(int nItem);
