...
{
	// If we have the mouse down, ignore this, so we don't adjust shot once we've clicked to Strike
...
float CSnooker::GetStrikeBallError(BOOL bWithRnd) // NOTE: this function is not used!!!
{
	...
	if(m_cueError >= 1) // semi-pro, pro
	{
		...
		/*
		softCloseThinShotError = 0;
		char buf[256];
		//sprintf(buf, "dd = %f, angErr = %f, powErr = %f", dd, angErr, powErr);
		sprintf(buf, "softCloseThinShotError = %f", softCloseThinShotError);
		ErrorMessage(g_wnd, buf);
		*/
	}
...
//	fRnd = ((rand()%100)-50) * 0.00008;
	...
	//char buff[80];
	//sprintf(buff, "(E_Rnd = %f), CutError = %f", E_Rnd, cutError);
	//ErrorMessage(m_parent, buff);
	/////////////////////////////////////////////////////////////////
...
//	fRnd = ((rand()%100)-50) * 0.00008;
	...
	{
		char buff[80];
		sprintf(buff, "(E_Rnd = %f), cushionError = %f", E_Rnd, cushionError);
		ErrorMessage(m_parent, buff);
	}
	*/
	...
	if(m_cueError == 2)	
		cushionError = cushionError * 0.5f; // 0.6f
	else if(m_cueError == 1)
		cushionError = cushionError * 0.5f;	// 0.6f;
	else
		cushionError = 0;
...
//	fRnd = ((rand()%100)-50) * 0.00008;
	...
	*/
	...
	{		
		...
		E_Rnd = E_Rnd + cushionError * powerErrorFactor * 1.4;

		E_Rnd = E_Rnd + hamperedByBallError * powerErrorFactor * 0.6;

		E_Rnd = E_Rnd + SideSpinError * 0.5;			// New Error for Side Spin (Added 6th March 07)
		
		
		E_Rnd *= 0.75;//0.70;//0.77;//0.88;//0.93; // -5% from 93%
