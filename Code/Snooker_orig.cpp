...
{	
	...
	switch(n)
		{
		case 0:// BLUE //
			   m_pColorScheme->m_tableCushionRed = _256TO1(62);
			   m_pColorScheme->m_tableCushionGreen = _256TO1(126);
			   m_pColorScheme->m_tableCushionBlue = _256TO1(232);
			   m_pColorScheme->m_tableSlateRed = _256TO1(31);
			   m_pColorScheme->m_tableSlateGreen = _256TO1(102);
			   m_pColorScheme->m_tableSlateBlue = _256TO1(219);
			   break;
...
void CCustomTable::PoolTableModifyMaterials(M3d_object *m_M3d_Table_ptr)
{
	...
	ChangeColorScheme(6, m_M3d_Table_ptr); // [+] Retrieve from a save file
...
// menus
	...
	// set menu option anti-alias
	if(g_gfxDetails.bOptionsAntiAliasing)
	{
		g_gfxDetails.bOptionsAntiAliasing = TRUE;
		if(menu)
		{
			subMenu = menu->GetSubMenu(OPTIONS_MENU);
			if(subMenu)
				subMenu->CheckMenuItem(OPTIONS_ANTI_ALIASING, MF_CHECKED | MF_BYPOSITION);
		}
	}
	else
	{
		g_gfxDetails.bOptionsAntiAliasing = FALSE;
		if(menu)
		{
			subMenu = menu->GetSubMenu(OPTIONS_MENU);
			if(subMenu)
				subMenu->CheckMenuItem(OPTIONS_ANTI_ALIASING, MF_UNCHECKED | MF_BYPOSITION);
		}
	}
	...
	if(g_WideScreenEnabled)
	{
		if(menu)
		{
			subMenu = menu->GetSubMenu(WIDESCREEN_MENU);
			if(subMenu)
				{
				subMenu->CheckMenuItem(WIDESCREEN_ENABLED, MF_CHECKED | MF_BYPOSITION);
				subMenu->CheckMenuItem(WIDESCREEN_DISABLED, MF_UNCHECKED | MF_BYPOSITION);
				}
		}
	}
	else
	{
		if(menu)
		{
			...
			if(subMenu)
				{
				subMenu->CheckMenuItem(WIDESCREEN_ENABLED, MF_UNCHECKED | MF_BYPOSITION);
				subMenu->CheckMenuItem(WIDESCREEN_DISABLED, MF_CHECKED | MF_BYPOSITION);
				}
		}
	}
...
void CSnooker::MouseMove(CPoint point)
{
	...
	if(m_cueBallSelect == 1)
	{
		if( MyTurn() )
		{
			...
			// -------------- //	
			m_mouseX+= ((float)x * MOUSE_CUE_SENSITIVY * screenWidthFactor);
			m_mouseY+= ((float)y * MOUSE_CUE_SENSITIVY * screenWidthFactor);
...
BOOL CSnooker::HamperedCueing(float x, float y, float w, float h)
{
	...
	for(n=0;n<1200/*500*/;n++) // not sure how many stepes are required? 500 is good for now (1*)
	{
		...
		if(m_M3d_Table_ptr == &m_customTable->m_M3d_ukTable)
			{
			if(P.x>(231.11*5 - 20) || P.x<(-231.11*5 + 20) ) if(P.z < (BALL_RADIUS * 3.0)) return TRUE; // x:
			if(P.y>(109.67*5 - 20) || P.y<(-109.67*5 + 20)) if(P.z < (BALL_RADIUS * 3.0)) return TRUE; // y:
			}
		else
			{
			if(P.x>((253*5-BALL_RADIUS*1.25)+BALL_RADIUS+15) || P.x<((-253*5+BALL_RADIUS*1.25)-BALL_RADIUS-15)) if(P.z < (BALL_RADIUS * 3.3)) return TRUE; // x:
			if(P.y>((126.5*5-BALL_RADIUS*1.25)+BALL_RADIUS+40) || P.y<((-126.5*5+BALL_RADIUS*1.25)-BALL_RADIUS-40)) if(P.z < (BALL_RADIUS * 3.3)) return TRUE; // y:
			}
...
BOOL CSnooker::FreeBallCheck(void)
{
	...
	{		
		...
		ballIdx = FreeBallTestStrike(n, D, D.Magnitude());
		//if(ballIdx != n)
		...
		ballIdx = FreeBallTestStrike(n, D2, D2.Magnitude());
...
void CSnooker::Process(float timeDelta)
{
	...
	if(m_firstTime)
	{
	((CGameAppView *)m_parent)->m_elCycle= EL_REST|EL_BALLS;
	}

	if(((CGameAppView *)m_parent)->m_elCycle&EL_REST);
	else{
...
//						m_bStrikeSend = FALSE;
			...
			if(m_bSeenSelectBall) {
				...
				//Referee();
				m_customTable->Referee(CTMSG_SIMEND);
			}
			//else 
			//	ReSpot();
...
#ifndef THE_POOL_CLUB
float CSnooker::GetStrikeBallError(BOOL bWithRnd) // NOTE: this function is not used!!!
{
...
//	int rnd = (m_rnd%100)-50;  // <-- OLD WAY!!!!!
	...
	//longAngleErrorFactor /= 3;
	longAngleErrorFactor = 1 - longAngleErrorFactor / 5;
	//longAngleErrorFactor += angleError * 10; 
	/*
	if(longAngleErrorFactor < 1.0) longAngleErrorFactor = 1.0;
	...
	*/
//	//////////////////////////////////////
//	// Calculate extra error - purpose to stop long sraight angled greens going in!
//	float extraError = 1.0f;
//	float straightness = (1.0f - angleError) - 0.15f; // 0.75 -> straight / -0.25 -> 90 degrees
//	extraError = m_strikeDist / (BALL_RADIUS * 2.0f * 15.0f); // 15 balls distance
//	extraError*=straightness;
/*
	//////////////////////////////////////////////////////////////////////////////////////////////
	//////////////////////////////////////////////////////////////////////////////////////////////

	float dist = (BALL_RADIUS * 2.0f * 15.0f) / m_strikeDist; // 15 balls distance;	
	//if(dist < 1) dist = 1;

	float extraError2Scale = 0.3;
	float extraError2 = extraError2Scale * dist;
	
	if(extraError2 < extraError2Scale)
			extraError2 = extraError2Scale;	
	if(extraError2 > 1) extraError2 = 1;
	
	// make long straight shots harder
	extraError2 *= ((1 - angleError)*3);
	
	if(extraError2 < extraError2Scale)
			extraError2 = extraError2Scale;
	if(extraError2 > 1) extraError2 =1;

	//float extraError3 = m_strikeDist / (BALL_RADIUS * 2.0f * 15.0f);
	//if(extraError3 < 1) extraError3 = 1;

	//extraError2 = 1;

	//extraError2 += (1 - angleError*angleError);
	//if(extraError2 > 1) extraError2 = 1;
...
*/
	//float extraError2;
	//float angleE = angleError;
	//if(angleE > 0.35) angleError *= 
	


	//if(straightness < -0.6) straightness = -0.6;

	//if(extraError < -1.5) extraError = -1.5;
	//if(extraError > 1.5) extraError = 1.5;
	//////////////////////////////////////
	...
	*/
	...
	else
	{
		...
		float comp_far = sqrt(dd);
		comp_far = sqrt(comp_far) * 0.2;
		float comp_near = dd*dd * 0.2;
		
		
		char buffer[80];
		sprintf(buffer, "angleError: %f, dd: %f", angleError, dd);
		ErrorMessage(m_parent, buffer);


		if(angleError > 1.0)
		{
			angleError = pow(angleError, 3);
		}

		angleError *= (comp_far + comp_near);
		...
		if(m_cueError == 2) // pro-level
		{
			//if(angleError > 40) angleError = 40;
			//if(angleError > 20) angleError = 20;
	...
	if(m_cueError == 1) // amatuer-level ?? <-------------- check against old code!
	{
		angleError *= 1.5;//0.25; // latest v1.6.2
	}
	else if(m_cueError == 2) // semi-pro-level
	{
		angleError *= 1.5;//0.25; // latest v1.6.2
	}
	else if(m_cueError == 3) // pro-level
	{
		//angleError *= 0.75; // was *1.0 before
		angleError *= 1.5; //1.75; // was *1.0 before
	}
	//angleError = 0;  //v1.6
	*/
	if(m_cueError >= 1)
	...
	*/
	...
	if(m_cueError == 2)
		E_Rnd *= 0.00008f;//E_Rnd *= 0.00018f;    ///// Scaling of Error.. You might want to increase this.
...
//	fRnd = ((rand()%100)-50) * 0.00008;
	...
	// Add on Close Distance ERror component, as it's tough to judge angle when we're reallyclose
	dd = (BALL_RADIUS*2.0f) / (m_strikeDist * m_strikeDist); // in no. of balls
...
//	}
	...
	//char buffy[80];
	//sprintf(buffy, "Error SideSpin = %f", SideSpinError);
	//sprintf(buffy, "tip[0]:%f, tip[1]:%f, tip[2]:%f", m_cuetip[0], m_cuetip[1], m_cuetip[2]);
	//ErrorMessage(m_parent, buffy);

/*
	// SNOOKER ONLY: Vertical Spin Error. The More Side spin we apply, the more error needs to be added to the shot //
	float VerticalSpinError = 0.0f;
	VerticalSpinError = m_cuetip[1] / 1.55f;
	VerticalSpinError = VerticalSpinError * m_cuetip[1] / 1.55f;
	VerticalSpinError = VerticalSpinError * VerticalSpinError;
	VerticalSpinError = VerticalSpinError * PowerFactor;
*/
...
//	fRnd = ((rand()%100)-50) * 0.00008;
	...
	*/
	...
	{		
		...
		E_Rnd = E_Rnd + (1-longAngleErrorFactor) * 0.0005;
	...
	if(g_build.gameType == GAMETYPE_SNOOKER)
	{
		switch(type)
		{
			case 0:
				...
				BALL_CUSHIONFACTOR = 0.8;
				break;

			case 1:
				BALL_ENERGYFACTOR =	0.98f;
				BALL_ENERGYFACTOR_N = 0.93f;
				BALL_ENERGYFACTOR_T = 0.93f;
				BALL_CUSHIONFACTOR = 0.85;
				break;
		}
	}
	else
...
void CSnooker::DrawTri(M3d_vtx pos, M3d_vtx rot, M3d_vtx scale, COLORREF rgba, float gAlpha)
{
	...
	pVtx[0].color = D3DCOLOR_RGBA(ri, gi, bi, alpha);
...
void CSnooker::MakeChalkMark(CBall *ball)
{
	...
	if(m_cuetip[1] < 0)
	{
		...
		if(m_bTurn1P) 
		{
			...
			{
				chalk_index = (m_player[plr1Idx].chalktype) & 0xff;
				chalks[chalk_idx].col = chalk_col[chalk_index];
			}
		}
		else
		{
			...
			{
				chalk_index = (m_player[plr2Idx].chalktype) & 0xff;
				chalks[chalk_idx].col = chalk_col[chalk_index];
			}
		}
...
void CSnooker::DrawBall(int n, BOOL bTrans)
{
	...
	if(g_build.gameType == GAMETYPE_SNOOKER) Ball_ptr = &m_M3d_Ball_ptr[0]; // snooker uses one ball mesh
	else {
			if(g_bGfxQuality==FALSE) // Low Quality.
			{
			if(m_customTable->m_type == CTTYPE_BOWLS)
